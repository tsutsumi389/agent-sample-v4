"""エヴァリュエーター: 直近ラウンドで実行されたステップ群を並列に判定する。

executor が status="running" にしたステップを asyncio.gather で並列評価し、それぞれに
retry / replan の予算管理を適用する。
- 空結果・打ち切りは LLM を呼ばず決定論的に retry / fail。
- パース全滅・例外時は pass (前進フォールバック — 評価器が壊れても全体は完走する)。
- retry 上限まで再実行しても未達成のステップは replan へエスカレーションする。replan の
  予算も尽きていれば fail にダウングレードして前進を保証する (routing.py は純関数のまま)。
- retry はステップを pending に戻し (次ラウンドで再実行)、step 専用 feedback を残す。
"""

import asyncio
import logging

from langchain_core.runnables import RunnableConfig
from langchain_core.messages import HumanMessage, SystemMessage

from app.agent.nodes.common import (
    EXECUTION_FAILED_MARKER,
    format_feedback_history,
    format_step_data,
    safe_stream_writer,
    screen_step_data,
)
from app.agent.parsing import RubricScores, VerdictSchema, structured_or_parse
from app.agent.prompts import EVALUATOR_SYSTEM, evaluator_user
from app.core.config import Settings

logger = logging.getLogger(__name__)


def make_evaluator_node(model, settings: Settings, screen_model):
    async def evaluator_node(state: dict, config: RunnableConfig) -> dict:
        plan = [dict(s) for s in state.get("plan") or []]
        running = [i for i, s in enumerate(plan) if s.get("status") == "running"]
        if not running:
            # 防御: 評価対象なし → 何も変えず routing が plan 状態で前進判断する
            return {}

        replan_budget_left = state.get("replan_count", 0) < settings.max_replans

        async def eval_one(idx: int) -> tuple[int, str, str, int]:
            # 例外を漏らさない契約 (executor.run_one と対称)。gather は return_exceptions=False の
            # ため、1件でも例外を出すと他コルーチンを巻き込んで node 全体が落ち、executor が
            # running にしたステップ群が未確定のまま消える。想定外の例外も pass で前進させる。
            step = plan[idx]
            try:
                result = step.get("result") or ""
                score = 0  # プレチェック retry / fail 経路ではスコアなし (可観測性用に 0)
                # 決定論プレチェック (LLM スキップ)
                if not result.strip() or result.startswith(EXECUTION_FAILED_MARKER):
                    verdict, feedback = (
                        "retry",
                        "前回の実行が失敗または空の結果でした。別のアプローチで再実行してください。",
                    )
                else:
                    # 構造化データは判定に必要な分だけへ絞ってから補助材料として渡す (値は変えず選別のみ)。
                    screened = await screen_step_data(
                        screen_model,
                        step.get("data"),
                        purpose=(
                            f"ステップ「{step['description']}」(要件: {step.get('instruction') or '—'})"
                            "の実行結果が目的・制約を満たしているかの評価"
                        ),
                        settings=settings,
                    )
                    # この時点の feedback_history は過去 retry の全指摘 (追記は後段 line 112-)。
                    # 再評価では全指摘を評価者へ渡し、各指摘の反映度の判断材料にする。
                    prior_feedback = format_feedback_history(step.get("feedback_history") or [])
                    parsed = await structured_or_parse(
                        model,
                        [
                            SystemMessage(content=EVALUATOR_SYSTEM),
                            HumanMessage(
                                content=evaluator_user(
                                    step_description=step["description"],
                                    step_instruction=step.get("instruction") or "",
                                    result=result,
                                    data=format_step_data(screened),
                                    prior_feedback=prior_feedback,
                                )
                            ),
                        ],
                        VerdictSchema,
                        use_structured=settings.supports_structured_output,
                        # 前進フォールバック: 評価器が壊れても完走するよう満点 pass にする。
                        fallback=VerdictSchema(scores=RubricScores(goal=5, accuracy=5, completeness=5)),
                    )
                    # ルーブリック採点 → verdict をコード側で導出 (閾値は決定論的でブレない)。
                    score = parsed.score
                    feedback = parsed.feedback
                    if parsed.flawed:
                        verdict = "replan"
                    elif score >= settings.eval_pass_threshold:
                        verdict = "pass"
                    else:
                        verdict = "retry"
                # 予算ダウングレード (決定論コード — 停止性の保証)
                # retry 上限まで再実行しても達成できなかったステップは、諦める (fail) 前に
                # 計画自体の立て直し (replan) を試みる。replan 予算も尽きていれば最終的に
                # fail へ落として前進を保証する (下段の replan ガードで処理される)。
                if verdict == "retry" and step.get("attempts", 0) > settings.max_step_retries:
                    verdict = "replan"
                if verdict == "replan" and not replan_budget_left:
                    verdict = "fail"
                return idx, verdict, feedback, score
            except Exception:
                logger.exception("ステップ評価に失敗したため pass で前進します (step=%s)", step.get("id"))
                return idx, "pass", "", 0

        verdicts = await asyncio.gather(*(eval_one(i) for i in running))

        failure_notes = list(state.get("failure_notes") or [])
        needs_replan = bool(state.get("needs_replan"))
        writer = safe_stream_writer()

        def _note(step: dict, feedback: str) -> str:
            desc = (step.get("description") or "")[:100]
            return f"ステップ{step.get('id')}「{desc}」: {feedback[:200] or '達成できませんでした'}"

        def _replan_note(step: dict, feedback: str) -> str:
            # planner が同じ達成不可能なタスクを再生成しないよう、「何回試みて・何が達成できな
            # かったか」を全指摘の履歴つきで言語化する。最新の feedback はまだ feedback_history
            # へ未追記なので末尾に足し、過去 retry の全指摘と1本にまとめて理由として渡す。
            desc = (step.get("description") or "")[:100]
            attempts = step.get("attempts", 0)
            history = [h for h in (step.get("feedback_history") or []) if h]
            if feedback:
                history.append(feedback[: settings.feedback_max_chars])
            issues = format_feedback_history(history) or "(具体的な指摘なし)"
            return (
                f"ステップ{step.get('id')}「{desc}」: {attempts}回試行しても達成できませんでした。\n"
                f"未解決の課題 (古い順):\n{issues}\n"
                "→ 同じタスクの再掲では達成できません。別のアプローチ・粒度で計画を立て直すこと。"
            )

        for idx, verdict, feedback, score in verdicts:
            step = plan[idx]
            if verdict == "pass":
                step["status"] = "done"  # done は再評価されないため履歴はそのまま残す
            elif verdict == "retry":
                step["status"] = "pending"  # 次ラウンドで再実行 (依存は既に done)
                # 今回の指摘を履歴へ積む。次回 executor は全指摘を、evaluator は全反映度を見る。
                history = list(step.get("feedback_history") or [])
                history.append(feedback[: settings.feedback_max_chars])
                step["feedback_history"] = history
            elif verdict == "replan":
                needs_replan = True
                # status は running のまま (再計画で置換)。planner に達成不能の理由を全履歴で渡す。
                failure_notes.append(_replan_note(step, feedback))
            else:  # fail: ステップを諦めて前進
                step["status"] = "failed"
                failure_notes.append(_note(step, feedback))
            writer(
                {
                    "status": f"ステップ {step['id']} を評価: {verdict} ({score}点)",
                    "phase": "evaluate",
                    "step": step["id"],
                    "verdict": verdict,
                    "score": score,
                }
            )

        return {"plan": plan, "failure_notes": failure_notes, "needs_replan": needs_replan}

    return evaluator_node
